...
async function buildSuite(runs: number): Promise<void> {
	...
	for (let runNumber = 1; runNumber <= runs; runNumber++) {
		...
		console.log(
			`clean full [${runNumber}/${runs}]: ${cleanSamples.at(-1)?.toFixed(3)}s`,
		);

		cachedSamples.push(await timed(() => run(build)));
		console.log(
			`cached full [${runNumber}/${runs}]: ${cachedSamples.at(-1)?.toFixed(3)}s`,
		);
	}
	...
	const results: Result[] = [
		...
		await sample("change to main.typ", runs, () =>
...
async function metadataSuite(runs: number): Promise<void> {
	...
	await run(build);
	const entries = await readdir(`${root}src/typst/blog`, {
		withFileTypes: true,
	});
	const posts = entries
		.filter(
...
switch (suite) {
	...
	default:
		throw new Error(
			`unknown suite ${JSON.stringify(suite)}; expected builds, profile, metadata, or all`,
		);
}
