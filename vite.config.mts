...
import { injectHead } from "./scripts/inject-head.mts";
...
function isResumeSource(file: string): boolean {
	return file.startsWith("resume/resume/");
}
...
async function buildChanges(changes: Change[]): Promise<boolean> {
	const files = changes.filter(
		(change): change is string => change !== fullBuild,
	);
	...
	if (
		...
		files.some((file) => !isKnownSource(file))
	) {
	...
	const rebuildAllBlogPosts =
		...
		sharedAssetChanged ||
		metadataChanged;
	const rebuildMain = mainSourceChanged || rebuildAllBlogPosts;
	...
	if (touchedHtml) {
		const files =
			rebuildMain || rebuildAllBlogPosts
				? undefined
				: posts
						.map((post) =>
							resolve(
								projectRoot,
								"out/blog",
								post.slice(blogRoot.length).replace(/\.typ$/, ".html"),
							),
						)
						.filter((file) => existsSync(file));
		await injectHead({ projectRoot, files });
	}
	return touchedHtml;
}
